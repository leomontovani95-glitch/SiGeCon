...
export default async function EditarRegraPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="p-6 max-w-2xl">
      ...
      <RegraForm id={regra.id} defaultValues={{ article: regra.article, item: regra.item ?? "", letter: regra.letter ?? "", description: regra.description, theme: regra.theme ?? "", defaultCommunicationType: regra.defaultCommunicationType ?? "", active: String(regra.active) }} />
    </div>
  );
}
