...
import { resetarSenha } from "../actions";

export default function ResetarSenhaBtn({ userId }: { userId: string }) {
  const action = resetarSenha.bind(null, userId);
  const [state, formAction, pending] = useActionState(action, undefined);

  return (
    <div className="mt-6 border-t border-gray-200 pt-6">
      <h3 className="text-sm font-semibold text-gray-700 mb-1">Redefinir Senha</h3>
      <p className="text-xs text-gray-500 mb-3">
        A senha será redefinida para o <strong>Número Funcional</strong> do usuário.
        O usuário poderá alterá-la em <em>Meu Perfil</em> após o próximo acesso.
      </p>
      {state?.success && (
